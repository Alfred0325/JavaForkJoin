...
public class Simulation{
    ...
    public Set<Pair<FAState,FAState>> Naive_DirectSimRelNBW(FiniteAutomaton omega1, FiniteAutomaton omega2) 
	{
		...
		DirectSimRelNBW_refine(isFinal, n_states, n_symbols, post, post_len, W);
...
private void DirectSimRelNBW_refine(boolean[] isFinal, int n_states, int n_symbols, int[][][] post, int[][] post_len, boolean[][] W)
{
		int sincechanged=0;
		while(true){
		    // System.out.println("Memeff Direct sim: Iterating main loop");
		    for(int p=0; p<n_states; p++)	
			for(int q=0; q<n_states; q++){
			    ++sincechanged;
			    if(W[p][q]){  // false remains false;
			    if(DirectSimRelNBW_Fail(p, q, n_symbols, post, post_len, W)) { W[p][q]=false; sincechanged=0; }
			    }
...
public Set<Pair<FAState,FAState>> MediatedRelNBW(FiniteAutomaton fa, Set<Pair<FAState, FAState>> frel,Set<Pair<FAState, FAState>> brel)
	 ...
	 * This is the current best speed-optimized version.
...
private boolean Segment_BLAB_defense(int p, int q, boolean[] isFinal, boolean[] isInit, int n_states, int n_symbols, int[][][] post, int[][] post_len, boolean[][] W, int la, int[] attack, int depth, int counter)
{
	...
	// If small enough <= 4GB then call HHK bw sim, else more memory efficient bitset naive
	int n_states = omega1.states.size();
	if(omega2!=null) n_states = n_states + omega2.states.size();
	HashSet<String> alphabet = new HashSet<String>();
	alphabet.addAll(omega1.alphabet);
	if(omega2!=null) alphabet.addAll(omega2.alphabet);
	int n_symbols = alphabet.size();
	if((long)n_states * (long)n_states * (long)n_symbols <= (long)800000000)
	   return(HHK_BackwardSimRelNBW(omega1, omega2));
	   else return(Bitset_Naive_BW_SimRelNBW(omega1, omega2));
    }
    ...
    public Set<Pair<FAState,FAState>> HHK_BackwardSimRelNBW(FiniteAutomaton omega1,FiniteAutomaton omega2){
		ArrayList<FAState> all_states = new ArrayList<FAState>();
		HashSet<String> alphabet = new HashSet<String>();
	...
	// If small enough <= 4GB then call HHK sim, else more memory efficient bitset naive
	int n_states = omega1.states.size();
	if(omega2!=null) n_states = n_states + omega2.states.size();
	HashSet<String> alphabet = new HashSet<String>();
	alphabet.addAll(omega1.alphabet);
	...
	if((long)n_states * (long)n_states * (long)n_symbols <= (long)800000000)
	   ...
	   else return(Bitset_Naive_DirectSimRelNBW(omega1, omega2));
    }
...
// -------------------------------------------------- Bitset Naive version of fw and bw simulation
