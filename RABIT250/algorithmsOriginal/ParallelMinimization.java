...
public class ParallelMinimization{
	...
	public FiniteAutomaton quotient(FiniteAutomaton fa, Set<Pair<FAState, FAState>> rel) {
	    // System.out.println("Quotienting aut, size: "+fa.states.size());
		...
		// System.out.println("Finished Quotienting aut, size: "+result.states.size());
		return result;
	}
	...
	// System.out.println("removed_trans_extra, trans: "+fa.trans);

	// Create cluster version of bw sim to avoid iterating over all states later
	TreeMap<FAState, TreeSet<FAState>> cluster_bw_rel = new TreeMap<FAState, TreeSet<FAState>>();
	Iterator<Pair<FAState,FAState>> bw_rel_it = bw_rel.iterator();
	while(bw_rel_it.hasNext()){
	    Pair<FAState,FAState> pair = bw_rel_it.next();
	    // Only consider bw larger states in fa2, not necessarily fa
	    if(fa2.states.contains(pair.getRight())){
		if(cluster_bw_rel.get(pair.getLeft()) == null) cluster_bw_rel.put(pair.getLeft(),new TreeSet<FAState>());
		cluster_bw_rel.get(pair.getLeft()).add(pair.getRight());
	    }
	}
		...
		while(state_it.hasNext()){
		    FAState state=state_it.next();
		    if(cluster_bw_rel.get(state)==null) continue; // Nothing to do if no states bw bigger than state
		    Iterator<String> sym_it = state.nextIt();
		    while(sym_it.hasNext()){
				String sym=sym_it.next();
				...
				while(deststate_it.hasNext()){
				    ...
				    if (exists_better_trans(state, sym, deststate, fw_rel, cluster_bw_rel.get(state))) 
					{
		...
		// System.out.println("Finished removed_trans_extra, trans: "+fa.trans);
		return result;
	}

	// Richard: Aux. test for removable_trans_extra

	    private boolean exists_better_trans(FAState state, String sym, FAState deststate, Set<Pair<FAState,FAState>> fw_rel, TreeSet<FAState> bw_bigger){

		Iterator<FAState> other_state_it = bw_bigger.iterator();
		while(other_state_it.hasNext()){
   		    ...
   		    if(other_state.getNext(sym) != null) 
			{
				...
				while(other_deststate_it.hasNext()){
				    ...
				    if(fw_rel.contains(new Pair<FAState,FAState>(deststate,other_deststate))) return true;
				}
			}
		}
		return false;
...
public FiniteAutomaton acc_state_decomposition(FiniteAutomaton system2, int i){
   ...
   public AutomatonPreprocessingResult Lightweight_Preprocess_Buchi(FiniteAutomaton system, FiniteAutomaton spec){
    ...
    public FiniteAutomaton Minimize_Finite(FiniteAutomaton system2, int i, boolean oneacc, boolean usejump, boolean usepebble){
		...
		do{
		    ...
		    // Only if lookahead >1 is specified
		    if(i>0){
		    // Repeated bw/fw quotienting w.r.t. direct bla sim.
   ...
   */
   ...
   public FiniteAutomaton saturate_Minimize_Finite(FiniteAutomaton system2, int i){
    ...
    public FiniteAutomaton Basic_Min_Finite(FiniteAutomaton system2){
	Set<Pair<FAState, FAState>> frel, brel;
	
	System.out.println("Before remove dead: # of Trans. "+system2.trans+", # of States "+system2.states.size()+".");
	FiniteAutomaton system=finite_removeDead(system2);
	System.out.println("After remove dead: # of Trans. "+system.trans+", # of States "+system.states.size()+".");

	System.out.println("Before oneacc: # of Trans. "+system.trans+", # of States "+system.states.size()+".");
	system = FiniteOneAcc(system);
	System.out.println("After oneacc: # of Trans. "+system.trans+", # of States "+system.states.size()+".");
	
	ParallelSimulation sim2 = new ParallelSimulation();
	frel = sim2.ForwardSimRelNBW(system,null);

	System.out.println("Before fw pruning: # of Trans. "+system.trans+", # of States "+system.states.size()+".");
	prune_fw(system,frel);
	System.out.println("After fw pruning: # of Trans. "+system.trans+", # of States "+system.states.size()+".");
        system=quotient(system, frel);
	system=finite_removeDead(system);
	
	brel = sim2.BackwardSimRelNBW(system,null);
	System.out.println("Before bw pruning: # of Trans. "+system.trans+", # of States "+system.states.size()+".");
	prune_bw(system,brel);
	System.out.println("After bw pruning: # of Trans. "+system.trans+", # of States "+system.states.size()+".");
	system=quotient(system, brel);
	system=finite_removeDead(system);
    ...
    // Mainly for testing/debugging
    public FiniteAutomaton Basic_Min_Finite_la(FiniteAutomaton system2, int la){
	Set<Pair<FAState, FAState>> frel, brel;
	...
	ParallelSimulation sim2 = new ParallelSimulation();
	frel = sim2.BLASimRelNBW(system,null,la);

        system=quotient(system, frel);
	system=finite_removeDead(system);
	
	brel = sim2.BLABSimRelNBW(system,null,la);
	system=quotient(system, brel);
	system=finite_removeDead(system);
