...
public class  TV {
	public static void main(String[] args) {	
		...
		if(args.length<4){
			...
			System.out.println("Usage: java -jar TV.jar size td ad alphabetsize filename");
			...
			System.out.println("alphabetsize=number of symbols, must be an integer >=1");
			...
			System.out.println("Example: java -jar TV.jar 100 1.9 0.8 2 test");
		}else{
			int size=Integer.parseInt(args[0]);
			float td=Float.parseFloat(args[1]);
			float fd=Float.parseFloat(args[2]);
			int alphabetsize = Integer.parseInt(args[3]);
			String name=args[4];
			FiniteAutomaton ba=genRandomTV(size, td, fd, alphabetsize);
