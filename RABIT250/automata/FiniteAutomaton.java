...
public class FiniteAutomaton{
	...
	public boolean saveAutomatonHKC2(String filename)
	{
		try{	
		    return saveAutomatonHKC2(new File(filename), filename);
		}catch(IOException e){
			System.out.println(e.getMessage());
			return false;
		}		
	}
	    public boolean saveAutomatonHKC2(File dest, String filename) throws IOException
	{
			BufferedWriter output;		
			FAState[] states = this.states.toArray(new FAState[0]);	
			output = new BufferedWriter(new FileWriter(dest));
			...
			output.write("@NFA\n");
			output.write("%Initial1 "+"q"+getInitialState().id+"\n");
			output.write("%Final ");
			Iterator<FAState> st_it=F.iterator();
			while(st_it.hasNext()){
				FAState st=st_it.next();
			...
			for(int i=0;i<states.length;i++){
				...
				while(label_it.hasNext()){
					...
					while(next_it.hasNext()){
						FAState s = next_it.next();
						String to_name = "q"+s.id;
						output.write(from_name+" "+"a"+label+" "+to_name+"\n");
					}
				}
			}
	...
	public boolean loadAutomatonfa(String filename)
	{
		try{
			return loadAutomatonfa(new File(filename));
		}catch(IOException e){
			System.out.println(e.getMessage());
			return false;
		}		
	}	
	public boolean loadAutomatonfa(File source) throws IOException 
	{
		Map<String, FAState> stMap=new TreeMap<String, FAState>(); 
		BufferedReader input = null;	
			input = new BufferedReader(new FileReader(source));
			...
			while(input.ready()){
				...
				if(trans)
				{
					...
					if(ss.length==3)
					{
						FAState fromState=stMap.get(ss[0]);
						if(fromState==null){
							fromState=createState();
							stMap.put(ss[0], fromState);
						}
						FAState toState=stMap.get(ss[1]);
						if(toState==null){
							toState=createState();
							stMap.put(ss[1], toState);
						}
						addTransition(fromState,toState,ss[2]);
						continue;
					}
					trans = false;
				}				
				newState=stMap.get(s);
				if(newState==null){
						newState=createState();
						stMap.put(s, newState);
				}
    	...
    	public boolean saveAutomatonfa(String filename)
	{
		try{	
			return saveAutomatonfa(new File(filename));
		}catch(IOException e){
			System.out.println(e.getMessage());
			return false;
		}		
	}
	public boolean saveAutomatonfa(File dest) throws IOException
	{
			BufferedWriter output;		
			FAState[] states = this.states.toArray(new FAState[0]);	
			output = new BufferedWriter(new FileWriter(dest));						
			...
			{
				Iterator<String> label_it = states[i].nextIt();
				String from_name = Integer.toString(states[i].id);
				...
				while(label_it.hasNext()){
					...
					{
						FAState s = next_it.next();
						String to_name = Integer.toString(s.id);					    
						output.write(from_name+" "+to_name+" "+label+"\n");						
					}
				}
			}
			Iterator<FAState> st_it=F.iterator();
